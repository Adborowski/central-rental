...
$("#marker-modal").attr("data-flat-id")
  ? openModal($("#marker-modal").attr("data-flat-id"))
  : console.log("nope");
...
function openModal(flatId) {
  ...
  $.each(jFlats, function (index, jFlat) {
    if (jFlat.flat_id == flatId) {
      ...
      $("#boxModalButtons").html(
        `
            <div style="text-decoration: none" data-object=` +
          jFlat.object_code +
...
$(".boxArrowButton").on("click", function () {
  ...
  if (photoId <= 1 && changeValue == -1) {
    ...
    photoId >= jFlats[flatId - 1].number_of_photos &&
    changeValue == 1
  ) {
